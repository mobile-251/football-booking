...
export default function HomeScreen() {
	...
	const filterVenues = useCallback(() => {
		...
		if (appliedSearch) {
			...
			result = result.filter((v) => v.name.toLowerCase().includes(query) || v.address?.toLowerCase().includes(query));
		}
								...
								{searchSuggestions.map((venue, index) => (
									...
									>
										...
										<View style={styles.suggestionContent}>
											...
											<Text style={styles.suggestionSubtitle}>{venue?.name || 'Sân bóng'}</Text>
										</View>
