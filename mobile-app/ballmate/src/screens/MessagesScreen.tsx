...
export default function MessagesScreen() {
	...
	const getFilteredConversations = () => {
		...
		if (searchQuery) {
			...
			result = result.filter(
				(c) => c.fieldName.toLowerCase().includes(query) || c.lastMessage.toLowerCase().includes(query)
			);
		}
	...
	const renderLoginPrompt = () => (
		<View style={styles.loginContainer}>
			...
			<Text style={styles.loginText}>Vui lòng đăng nhập để xem tin nhắn của bạn</Text>
	...
	const renderConversation = ({ item }: { item: Conversation }) => (
		<TouchableOpacity style={styles.conversationItem} onPress={() => handleConversationPress(item)}>
			<View style={styles.avatarContainer}>
				<Image source={{ uri: item.fieldImage }} style={styles.avatar} />
			...
			<View style={styles.conversationContent}>
				<Text style={[styles.fieldName, item.unread && styles.fieldNameUnread]}>{item.fieldName}</Text>
						...
						<View style={styles.filterTabs}>
							{FILTERS.map((filter) => (
								...
								>
									...
									<Text style={[styles.filterTabCount, activeFilter === filter.key && styles.filterTabCountActive]}>
